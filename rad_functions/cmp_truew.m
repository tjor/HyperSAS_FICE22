...
function [adir, tdir, tspd, nw, nwam, nwpm, nwf] = cmp_truew(sel=3, crse, cspd, wdir, zlr, hd, wspd, wmis)


num = length(crse);
tdir = tspd = nan(size(crse));



if num != length(cspd) | num != length(wdir) | num != length(hd) | num != length(wspd)
	disp('input vectors should have the same length');
	keyboard
	exit
endif 
