function [L2] = hsas_filter_sensors_using_Lu_data(L1_f, L2, XX, vza_nm)
...
# no filter case
    if XX==100;
        ...
        iLu = 1:length(L1_f.instr.Lu.data(:,1));
...
##### find lower XX% of Lu data (Marco fa cosi')

    [tmp iwv] = intersect(L2.wv, 450:650);

    tmp = mean(L1_f.instr.Lu.data(:,iwv)');

    iLu = find(tmp <= prctile(tmp',XX));
